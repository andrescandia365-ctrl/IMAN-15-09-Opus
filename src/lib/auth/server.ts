...
function isLanDevOrigin(origin: string): boolean {
  try {
    const u = new URL(origin);
    if (u.protocol !== "http:" || u.port !== "8080") return false;
    const h = u.hostname;
    return (
...
const baseURL = explicitBaseURL ?? {
  ...
  // (not only the preview wildcard). LAN hosts: phone testing on the same box.
  allowedHosts: [
    ...previewAllowedHosts,
    "localhost",
    "127.0.0.1",
...
const trustedOriginsStatic: string[] = explicitBaseURL

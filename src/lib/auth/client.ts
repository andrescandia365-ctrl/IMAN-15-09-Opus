...
import { unlockFloor } from "@/lib/floor-lock";
import { clearCachedSession } from "@/lib/local-db";
...
export const authClient = createAuthClient({
  ...
  fetchOptions: {
    ...
    onSuccess(ctx) {
      // http://192.168.x.x is not a secure context, so `__Host-` cookies never
      // stick. Keep the session on the bearer path (same as live preview).
      if (!httpLanNeedsBearer()) return;
      const header = ctx.response.headers.get("set-auth-token");
      if (header) {
        setBearerToken(header);
        return;
      }
      const data = ctx.data as { token?: string } | null;
      if (data && typeof data.token === "string" && data.token) setBearerToken(data.token);
    },
  },
});
...
/** True on http://LAN (not localhost): Secure `__Host-` cookies are refused. */
function httpLanNeedsBearer(): boolean {
...
/** After email sign-in on a LAN IP, persist the session token for getSession. */
export function captureLanSessionToken(token: string | null | undefined): void {
  if (!token || !httpLanNeedsBearer()) return;
...
export async function signOut(redirectTo = "/"): Promise<void> {
  unlockFloor();
  void clearCachedSession().catch(() => undefined);
